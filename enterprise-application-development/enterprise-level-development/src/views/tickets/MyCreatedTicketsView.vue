...
// 确认工单已解决
const confirmResolved = async (id: number | string) => {
  try {
    await ticketStore.updateStatus(id, { status: 'CLOSED' })
    await fetch() // 刷新列表
  } catch (error) {
...
const resetFilters = () => {
  filters.status = ''
  filters.creatorId = auth.user?.id
          ...
          <div class="card-footer">
            ...
            <div class="actions" @click.stop>
              <a-button
                ...
                type="primary"
                size="small"
                @click="confirmResolved(ticket.id)"
              >
                确认解决
              </a-button>
            </div>
